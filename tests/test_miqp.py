...
over every integer point of a model small enough to enumerate**. A node bound
here is not rigorous -- there is no Neumaier-Shcherbina equivalent for the QP
relaxation -- so agreement with an exhaustive search is the only thing that
actually establishes the search is not cutting off the optimum.
"""
...
def test_the_bound_is_reported_as_not_rigorous():
    """The LP path's node bound is made safe by Neumaier-Shcherbina; this one
    is whatever the first-order QP reports. Saying so is the difference between
    a limitation and a false claim."""
    p = target_miqp(0)
    s = solve_miqp(p.copy(), MIQPParams(time_limit=120))
    assert s.info["bound_is_rigorous"] is False
